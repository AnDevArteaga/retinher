  ...
  const content =
    splitBy === "chars"
      ? text.split("").map((char, i) => (
          <span key={i} className="reveal-unit inline-block" style={{ whiteSpace: char === " " ? "pre" : "normal" }}>
            {char}
          </span>
        ))
      : text.split(" ").flatMap((word, i, arr) => [
          ...
          ...(i < arr.length - 1 ? [<span key={`s-${i}`} className="inline-block w-[0.35em]" aria-hidden />] : []),
        ]);

  return (
    <Tag ref={containerRef as React.RefObject<HTMLSpanElement>} className={className}>
      {content}
    </Tag>
  );
}
